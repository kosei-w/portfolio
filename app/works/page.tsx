      ...
      <main className="pt-20">
        ...
        <div className="px-8 md:px-14 section-py pb-0 border-b border-[var(--border)]">
          <div className="max-w-[1200px] mx-auto pb-16">
            <p className="eyebrow text-[var(--muted)] mb-10">WORKS</p>
            <h1
        ...
        <section className="px-8 md:px-14 section-py">
          <div className="max-w-[1200px] mx-auto">
            ...
            {works.map((work) => (
              <article key={work.id} className="mb-28">
                ...
                <ScrollReveal className="flex flex-wrap items-baseline gap-6 mb-12 pb-8 border-b border-[var(--border)]">
                  ...
                  <span className="eyebrow text-[var(--muted)]">{work.tag}</span>
                  <h2
                ...
                {/* Before / After — 打ち消し線 vs 矢印+太字 */}
                <ScrollReveal className="grid md:grid-cols-2 gap-0 mb-16 border border-[var(--border)]">
                  {/* BEFORE */}
                  <div className="p-8 md:p-10 md:border-r border-[var(--border)]">
                    <p className="eyebrow text-[var(--muted)]/60 mb-8">BEFORE</p>
                    <ul className="space-y-0">
                      {work.before.map((item) => (
                        <li
                          key={item}
                          className="py-4 border-b border-[var(--border)]/60 text-sm leading-relaxed"
                          style={{
                            color: 'var(--muted)',
                            opacity: 0.55,
                  ...
                  <div className="p-8 md:p-10">
                    <p className="eyebrow text-[var(--gold)] mb-8">AFTER</p>
                    <ul className="space-y-0">
                      {work.after.map((item) => (
                        <li
                          key={item}
                          className="flex items-start gap-3 py-4 border-b border-[var(--border)]"
                        >
                          <span
                            className="font-display font-bold shrink-0 mt-0.5 text-sm"
                            style={{ color: 'var(--gold)' }}
                          >
                            →
                          </span>
                          <span className="font-display font-bold text-[var(--ink)] text-sm leading-relaxed">
                            {item}
                          </span>
                        </li>
                      ))}
                ...
                <ScrollReveal className="border-l-2 border-[var(--gold)] pl-8">
                  ...
                  <p className="eyebrow text-[var(--muted)]">— {work.quoteBy}</p>
                </ScrollReveal>
              </article>
            ))}
            ...
            <ScrollReveal className="border-t border-[var(--border)] pt-16">
              <p className="eyebrow text-[var(--muted)] mb-8">COMING SOON</p>
              <p
        ...
        <ScrollReveal className="border-t border-[var(--border)] bg-[var(--ink)] px-8 md:px-14 section-py">
          <div className="max-w-[1200px] mx-auto">
            <p className="eyebrow text-[var(--gold)] mb-16">PROCESS</p>
            <h2
            ...
            <ScrollReveal stagger className="border-t border-white/10">
              {process.map((item) => (
                <div
                  key={item.step}
                  className="flex flex-col md:flex-row md:items-start gap-4 md:gap-16 py-8 border-b border-white/10"
                >
